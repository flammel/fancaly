...
import { errorValue, numericValue } from "./evaluate";
...
function runTest(
  ...
  test(value + " " + from + " to " + to, () => {
    expect(convert(numericValue(value, getUnit(from) as Unit), getUnit(to) as Unit)).toEqual(
      error ? errorValue(expected) : numericValue(expected, getUnit(to) as Unit),
    );
  });
}
...
runTest("1", "mm", "g", "Cannot convert unit mm to g.", true);
